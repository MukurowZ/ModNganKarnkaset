<template>
    <div>
        <ProductCarousel v-if="image!=null" :images="image">Sidebar content here</ProductCarousel>
    </div>
</template>
<script>
export default {
    ...
    methods: {
        getAllImg(e){
            console.log(e)
            axios.get("/api/img_set/" + e).then(response => {
                this.setImgData(response.data);
            });
        },
        setImgData(e){
            for(var i=0; i<e.length; i++){
                this.image[i] = e.imgid[i].path;
            }
            this.img = e;
        },
    },
