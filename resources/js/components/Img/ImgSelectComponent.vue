      ...
      >
        <template height="104px" slot="singleLabel" slot-scope="props">
          ...
          <span class="option__name">
            <span class="option__id">เลือกอัลบั้ม: {{ props.option.id }}</span>
            ชื่ออัลบั้ม: {{ props.option.name }}
          </span>
        </template>
        <template slot="option" slot-scope="props" class="d-flex bd-highlight mb-3">
          <img height="104px" class="option__image p-2 bd-highlight" v-lazy="props.option.img" />
          <div class="option__name align-self-center p-2 bd-highlight">
  ...
  methods: {
    ...
    removePrime() {
      ...
      if (b != null) {
        axios
          .get("/api/img_set/" + b)
          .then(response => this.setImg_setData(response.data));
      } else return null;
    },
    setImg_setData(e) {
      this.img_set = {
